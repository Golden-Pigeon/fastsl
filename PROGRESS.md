...
with swanboard's `/api/v1` and serves the swanboard Vue UI (embedded), so the UI is identical.
Single command: `fastsl --logdir <swanlog dir> [--port 5092] [--host] [--cache-dir]`.

The frontend is a **git submodule** (`frontend/`) pointing at a fork of
`SwanHubX/SwanLab-Dashboard` (branch `fastsl`), so the UI can be customized and still synced from
upstream. `build.rs` runs the Vite build (`npm run build.release` → `frontend/swanboard/template/`)
and `rust-embed` embeds that output. See "Frontend workflow" below.
...
## Source layout (`src/`)
- `main.rs` — clap args, axum router, embedded assets (`rust_embed` of `frontend/swanboard/template/`, built by `build.rs`), SPA fallback, `SwanLab-Version`/`SwanLab-Process-Time` headers, `AppState { logdir, summary_cache, ui }`, startup summary prewarm, `notify` watcher + 30s polling fallback.
- `build.rs` — before compiling, runs the frontend Vite build in the `frontend/` submodule (`npm install` if `node_modules` absent, then `npm run build.release`), producing the bundle that `FrontendAssets` embeds. `rerun-if-changed` on `frontend/vue`, `package.json`, `vite.config.js` so plain Rust edits don't re-run npm. `FASTSL_SKIP_FRONTEND_BUILD=1` reuses an existing bundle without Node.
- `frontend/` — git submodule → fork of `SwanHubX/SwanLab-Dashboard` (branch `fastsl`); Vue3+Vite source in `frontend/vue/`, Vite output `frontend/swanboard/template/` (gitignored upstream, generated by `build.rs`).
...
## Frontend workflow (submodule + build.rs)
The UI is a git submodule `frontend/` → fork `Golden-Pigeon/SwanLab-Dashboard` (branch `fastsl`,
forked from `SwanHubX/SwanLab-Dashboard`). `build.rs` runs `npm run build.release` there and
`rust-embed` embeds `frontend/swanboard/template/`.

- **First clone:** `git submodule update --init --recursive` (else `build.rs` errors with that hint).
- **Customize the UI:** edit `frontend/vue/src/**`, then in `frontend/` commit + push to the fork's
  `fastsl` branch; back in the main repo `git add frontend` to move the submodule pointer. `cargo
  build` rebuilds the bundle automatically (npm build ~11s; only re-runs when frontend files change).
- **Sync upstream:** in `frontend/`, `git remote add upstream https://github.com/SwanHubX/SwanLab-Dashboard`,
  `git fetch upstream`, `git merge upstream/main`, resolve conflicts in `vue/`, push `fastsl`, then
  bump the pointer in the main repo.
- **API coupling (important):** the Rust `/api/v1` is a byte-for-byte replica of swanboard **0.1.10b2**.
  After syncing a newer frontend, re-run the contract check (`tests/dump_golden.py`) — if the upstream
  API shape changed, the Rust handlers must be updated to match. Bump deliberately, not automatically.
- **Node-less build:** `FASTSL_SKIP_FRONTEND_BUILD=1 cargo build` reuses an already-built bundle.
